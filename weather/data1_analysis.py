...
def hum_curve(data):
    ...
    hum_min = min(hum)
    hum_min_hour = hum[hum.index(hum_min)]  #求最低相对湿度
    x = []
    y = []
    for i in range(0, 24):
        x.append(i)
    ...
    plt.plot(x, y, color='blue')  #点出每个时刻的相对湿度
...
def main():
    ...
    print(data1)
    tem_curve(data1) #温度曲线图
    hum_curve(data1) #湿度曲线图
    air_curve(data1) #空气质量柱状图
    wind_radar(data1) #风向雷达
    corr_tem_hum(data1) #温湿度相干性分析
